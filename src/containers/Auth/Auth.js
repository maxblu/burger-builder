...
class Auth extends Component {
  ...
  render() {
    ...
    if (this.props.error) {
      error_mess = <p>{this.props.error.message}</p>;
    }
    ...
    return (
      <div className={classes.Auth}>
        <form onSubmit={this.submitHandler}>
          {form}
          {error_mess}
          <Button btnType="Success" disabled={!(this.state.controls.email.valid && this.state.controls.password.valid)}>
            SUBMIT
          </Button>
        </form>
        <Button btnType="Danger" clicked={this.switchAuthModeHandler}>
          SWITCH TO {this.state.isSingUp ? 'SIGN IN' : 'SIGN UP'}{' '}
        </Button>
      </div>
    );
  }
