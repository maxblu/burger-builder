...
export const auth = (email, password, method) => {
  return (dispatch) => {
    ...
    console.log(method);
    let url = 'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyAyunekmkqwDPhszx4-Hwy97_VGzlzP9kA';

    if (!method) {
      url = 'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAyunekmkqwDPhszx4-Hwy97_VGzlzP9kA';
    }
    ...
    axios
      ...
      .then((resp) => {
        console.log(resp);
        const expirationDate = new Date(new Date().getTime() + parseInt(resp.data.expiresIn) * 1000);
